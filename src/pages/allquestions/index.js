...
const AllQuestionsPage = () => {
    ...
    useEffect(() => {
        const fetchQuestions = async () => {
            setLoading(true);
            try {
                const res = await fetch(`/api/allquestions?page=${page}&limit=30`);
    ...
    const filtered = useMemo(() => {
        ...
        return (questions || []).filter((item) => {
            const okSearch = !q || item.title?.toLowerCase().includes(q) || item.topics?.join(',').toLowerCase().includes(q);
            const okDiff = difficulty === 'all' || (item.difficulty || '').toLowerCase() === difficulty;
